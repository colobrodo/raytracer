...
struct Solid {
    ...
    __host__ __device__ float hit(const Ray &ray, Vec3 &normal) const {
        // AABB collision for mesh
...
struct Scene {
    ...
    __host__ __device__ bool hit(const Ray &ray, RaycastResult &result) const {
        ...
        if(found_collision) {
            ...
            result.normal = normal_of_hit_point;
        }
